...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', ({ roomId, playerName, emoji }) => {
    ...
    if (room.gameState.phase !== 'lobby') return socket.emit('error', { message: 'Game has already started. Wait for the next round!' });
    if (!playerName || playerName.trim().length === 0) return socket.emit('error', { message: 'Player name is required.' });

    const cleanName = playerName.trim().substring(0, 16);
    if (room.players.find(p => p.name.toLowerCase() === cleanName.toLowerCase())) {
      return socket.emit('error', { message: `"${cleanName}" is already taken in this room. Choose another name.` });
    }
    ...
    io.to(id).emit('player-joined', { playerName: cleanName, emoji: emoji || '😄' });
    broadcastRoom(id);
  ...
  socket.on('pick-task', ({ type, questionText }) => {
    ...
    const isCurrentPlayer = socket.data.playerName === room.gameState.currentPlayer;
...
// ── REST endpoints ────────────────────────────────────────────────────────────
app.get('/', (req, res) => res.json({ status: 'IGNITE server running 🔥', rooms: rooms.size }));
app.get('/health', (req, res) => res.json({ ok: true, rooms: rooms.size, players: [...rooms.values()].reduce((a, r) => a + r.players.length, 0) }));
